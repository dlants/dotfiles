...
export type { LogFn, ThreadFn } from "./magenta-sdk/index.ts";
import type { LogFn, ThreadFn } from "./magenta-sdk/index.ts";
...
export async function getChangedPaths(
  ...
  if (stop) {
    add(await git(repo, "diff", "--name-only", `${start}..${stop}`));
  } else {
    add(await git(repo, "diff", "--name-only", start));
    add(await git(repo, "ls-files", "--others", "--exclude-standard"));
  }
  return [...paths].sort();
}
